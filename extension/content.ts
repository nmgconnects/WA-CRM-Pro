...
/**
 * WA-CRM Pro: Advanced Control Center
...
function createIntelSidebar() {
  ...
  sidebar.className = 'wa-crm-sidebar-hidden'; 
  ...
  intelStyles.innerText = `
    ...
    .status-pill { padding: 6px 12px; border-radius: 10px; font-size: 10px; font-weight: 800; text-transform: uppercase; border: 1px solid #f1f5f9; cursor: pointer; transition: all 0.2s; margin-bottom: 4px; display: inline-block; width: 100%; text-align: center; box-sizing: border-box; }
  ...
  document.querySelectorAll('.nav-tab').forEach(tab => {
    tab.addEventListener('click', (e) => {
        ...
        renderTabContent((e.target as HTMLElement).getAttribute('data-tab') || 'intel');
    });
  });
...
function renderTabContent(tab: string) {
    ...
    const contactName = (header?.querySelector('span[title]') as any)?.title || 'Selected Contact';
    ...
    switch(tab) {
        ...
        case 'sales':
            ...
            document.getElementById('btn-update-pipeline')?.addEventListener('click', () => {
                ...
                const value = (document.getElementById('deal-value-input') as HTMLInputElement).value;
        ...
        case 'auto':
            ...
            container.innerHTML = `
                ...
                <div class="intel-card">
                    <span style="font-size: 9px; font-weight: 900; color: #94a3b8; text-transform: uppercase; display: block; margin-bottom: 16px;">Schedule Messages</span>
                    <input type="datetime-local" id="msg-schedule-time" class="tool-input" />
                    <textarea id="msg-schedule-text" class="tool-input" placeholder="Schedule message to ${contactName}..."></textarea>
                    <button id="btn-schedule-msg" class="action-btn-sm" style="background: #6366f1;">Queue Message</button>
                </div>
            `;
            ...
            document.getElementById('btn-save-auto')?.addEventListener('click', () => {
                const keyword = (document.getElementById('auto-reply-keyword') as HTMLInputElement).value;
                const rows = document.querySelectorAll('.response-row');
                const responses = Array.from(rows).map(row => ({
                        text: (row.querySelector('.response-text') as HTMLTextAreaElement).value,
                        delay: parseInt((row.querySelector('.response-delay') as HTMLInputElement).value) || 0
            ...
            document.getElementById('btn-schedule-msg')?.addEventListener('click', () => {
                const time = (document.getElementById('msg-schedule-time') as HTMLInputElement).value;
                const text = (document.getElementById('msg-schedule-text') as HTMLTextAreaElement).value;
                chrome.runtime.sendMessage({ type: 'SCHEDULE_MESSAGE', payload: { time, text, recipient: contactName } }, () => showNotification('Message Scheduled!'));
            });
            break;
        case 'tools':
            container.innerHTML = `
                ...
                <div class="intel-card">
                    <span style="font-size: 9px; font-weight: 900; color: #94a3b8; text-transform: uppercase; display: block; margin-bottom: 16px;">Broadcast Campaigns</span>
                    <select id="campaign-segment" class="tool-input">
                        <option>Current Active Pipeline</option>
                        <option>April Cold Outreach</option>
                        <option>Q1 Customer Success</option>
                    </select>
                    <button id="btn-launch-campaign" class="action-btn-sm" style="background: #f43f5e;">⚡ Launch Campaign</button>
                </div>
            `;
            document.getElementById('btn-direct-chat')?.addEventListener('click', () => {
                const phone = (document.getElementById('direct-chat-phone') as HTMLInputElement).value;
                if (phone) {
                    window.open(`https://web.whatsapp.com/send?phone=${phone.replace(/\D/g, '')}`, '_blank');
                    showNotification('Redirecting...');
                }
            });
            document.getElementById('btn-launch-campaign')?.addEventListener('click', () => {
                const segment = (document.getElementById('campaign-segment') as HTMLSelectElement).value;
                chrome.runtime.sendMessage({ type: 'LAUNCH_CAMPAIGN', payload: { segment } }, () => showNotification('Campaign Started!'));
            });
            break;
    }
}
...
async function runAIIntelligence() {
    ...
    try {
        ...
        if (messages.length === 0) {
            container.innerHTML = `<p style="text-align: center; color: #94a3b8; font-size: 12px; padding: 20px;">No conversation data found to analyze.</p>`;
            return;
        }

        const prompt = `
            Analyze the following WhatsApp conversation with ${messages[0].sender}.
            Provide a JSON response with:
            - leadScore: 0-100 number
            - sentiment: "Positive", "Neutral", or "Negative"
            - strategy: A short 1-2 sentence follow-up advice
            - replyOptions: An array of 3 suggested short replies
        ...
        container.innerHTML = `
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 24px;">
                <div class="intel-card" style="margin-bottom: 0;">
                    ...
                    <p class="score-badge" style="margin: 8px 0 0 0;">${data.leadScore || '??'}</p>
                </div>
                <div class="intel-card" style="margin-bottom: 0;">
                    <span style="font-size: 9px; font-weight: 900; color: #94a3b8; text-transform: uppercase;">Sentiment</span>
                    <p style="font-size: 16px; font-weight: 900; color: ${data.sentiment === 'Positive' ? '#10b981' : data.sentiment === 'Negative' ? '#f43f5e' : '#64748b'}; margin: 8px 0 0 0;">${data.sentiment || 'Neutral'}</p>
                </div>
            </div>
            <div class="intel-card">
                ...
                <p style="font-size: 12px; color: #475569; line-height: 1.7; font-style: italic; margin: 0;">
                    ${data.strategy || 'Keep engaging with the prospect to move them down the funnel.'}
                </p>
            </div>
        `;

        const replyContainer = document.getElementById('intel-replies');
        if (replyContainer && data.replyOptions) {
            replyContainer.innerHTML = data.replyOptions.map((opt: string) => `<div class="reply-chip">"${opt}"</div>`).join('');
            attachChipListeners();
        }

    } catch (error) {
        console.error('AI Intelligence Error:', error);
        container.innerHTML = `<p style="color: #f43f5e; font-size: 11px; text-align: center; padding: 20px;">Analysis failed. Please try again.</p>`;
    }
}
...
function scrapeChat() {
    ...
    const allTextBlocks = messageElements.length > 0 ? messageElements : document.querySelectorAll('[data-pre-plain-text]');

    allTextBlocks.forEach((el) => {
        const textEl = el.querySelector('.copyable-text span');
        if (textEl) {
            const senderInfo = el.getAttribute('data-pre-plain-text') || (el.classList.contains('message-in') ? 'Contact' : 'Me');
            const sender = senderInfo.includes(']') ? senderInfo.split(']')[1].replace(':', '').trim() : senderInfo;
            messages.push({
                sender: sender,
    ...
    return messages.slice(-15); // Return last 15 messages
}
...
function sendMessage(text: string) {
    ...
    if (input) {
        ...
        setTimeout(() => {
            ...
            if (sendBtn) {
                (sendBtn.closest('button') as HTMLButtonElement).click();
            }
        }, 100);
    }
}
...
function toggleIntelSidebar() {
  ...
  if (sidebar) {
    ...
    if (!sidebar.classList.contains('wa-crm-sidebar-hidden')) {
...
function attachChipListeners() {
    document.querySelectorAll('.reply-chip').forEach(chip => {
        chip.addEventListener('click', () => {
            const hChip = chip as HTMLElement;
            const input = document.querySelector('footer div[contenteditable="true"]') as HTMLElement;
            if (input) {
                input.focus();
                document.execCommand('insertText', false, hChip.innerText.replace(/"/g, ''));
            }
        });
    });
}
...
function setupMutationObserver() {
  const observer = new MutationObserver(() => {
    ...
    const header = document.querySelector('#main header');
    if (header) {
      const currentChat = (header.querySelector('span[title]') as any)?.title || '';
...
let autoReplyRules: { keyword: string, responses: { text: string, delay: number }[] }[] = [];

async function loadAutoReplyRules() {
    const data = await chrome.storage.local.get('auto_reply_rules');
    autoReplyRules = (data as any).auto_reply_rules || [];
}
...
function detectIncomingMessages() {
    ...
    const text = (textEl as HTMLElement).innerText.toLowerCase();
    ...
    autoReplyRules.forEach(rule => {
        if (text.includes(rule.keyword)) {
            console.log('WA-CRM Pro: Auto-Reply Triggered for keyword:', rule.keyword);
            
            // Log trigger to analytics
            chrome.runtime.sendMessage({ 
                type: 'LOG_TRIGGER', 
                payload: { keyword: rule.keyword, contactName: lastChatName || 'Unknown Contact' } 
            });
            ...
            rule.responses.forEach((resp, index) => {
                ...
                setTimeout(() => {
                    ...
                    if (index === rule.responses.length - 1) {
                        showNotification(`Auto-Reply Sequence Completed: ${rule.keyword}`);
                    } else {
                        showNotification(`Auto-Reply: Sending part ${index + 1}...`);
                    }
